...
// LEARNER NAVIGATION
const LEARNER_ITEMS = [
  { id: 'dashboard', name: 'Dashboard', path: '/learner/dashboard', icon: LayoutDashboard },
  { id: 'learning', name: 'Learning', path: '/learner/learning-path', icon: Compass },
  { id: 'ai-tools', name: 'AI Tools', path: '/learner/materials', icon: Sparkles },
  { id: 'competency', name: 'Competency', path: '/learner/competencies', icon: Target },
  { id: 'analytics', name: 'Analytics', path: '/learner/analytics', icon: BarChart3 }
];

// TRAINER NAVIGATION
const TRAINER_ITEMS = [
  { id: 'dashboard', name: 'Studio Home', path: '/trainer/dashboard', icon: LayoutDashboard },
  { id: 'materials', name: 'Material Mgmt', path: '/trainer/materials', icon: FileText },
  { id: 'assessments', name: 'Assessments', path: '/trainer/assessments', icon: HelpCircle }
];

// ADMIN NAVIGATION
const ADMIN_ITEMS = [
  { id: 'dashboard', name: 'Analytics Console', path: '/admin/dashboard', icon: LayoutDashboard },
  { id: 'workforce', name: 'Workforce', path: '/admin/users', icon: User }
];
...
const SECTIONS_CONFIG = {
  learning: {
    ...
    defaultPath: '/learner/learning-path',
    parentName: 'Dashboard',
    parentPath: '/learner/dashboard',
    items: [
      { id: 'my-learning-path', name: 'My Learning Path', path: '/learner/learning-path', icon: Compass, desc: 'Sequential stage-by-stage journey' },
      { id: 'courses', name: 'Courses', path: '/learner/courses', icon: BookOpen, desc: 'iGOT Karmayogi integrated modules' },
      { id: 'assessments', name: 'Assessments', path: '/learner/quiz', icon: HelpCircle, desc: 'Validated diagnostic tests' }
    ]
  },
  'ai-tools': {
    id: 'ai-tools',
  ...
  competency: {
    ...
    defaultPath: '/learner/competencies',
    parentName: 'Dashboard',
    parentPath: '/learner/dashboard',
    items: [
      { id: 'skill-gap', name: 'Skill Gap', path: '/learner/competencies', desc: 'Identified deficit priorities' },
      { id: 'competency-assessment', name: 'Competency Assessment', path: '/learner/quiz', icon: HelpCircle, desc: 'Skill validation checkpoint' },
      { id: 'improvement-plan', name: 'Improvement Plan', path: '/learner/learning-path', icon: Compass, desc: 'Personalized closing roadmap' }
    ]
  },
  analytics: {
    id: 'analytics',
    ...
    defaultPath: '/learner/analytics',
    parentName: 'Dashboard',
    parentPath: '/learner/dashboard',
    items: [
      { id: 'learning-analytics', name: 'Learning Analytics', path: '/learner/analytics', icon: BarChart3, desc: '6-month skill growth curves' }
    ]
  }
};
...
export default function Navbar({ onToggleSidebar, pageTitle }) {
  ...
  // Determine role-based navigation and prefix
  const role = user?.role || 'learner';
  const rolePrefix = `/${role}`;
  
  let LEVEL_1_ITEMS = LEARNER_ITEMS;
  if (role === 'trainer') LEVEL_1_ITEMS = TRAINER_ITEMS;
  if (role === 'admin') LEVEL_1_ITEMS = ADMIN_ITEMS;

  // Determine if we are on Level 1 (Dashboard / Home) or inside a Level 2 Section
  const pathname = location.pathname;
  const isLevel1 = pathname === `${rolePrefix}/dashboard` || pathname === rolePrefix || pathname === '/';
  ...
  const getCurrentSection = () => {
    ...
    // Only learners have these complex section drill-downs right now
    if (role === 'learner') {
      if (pathname.startsWith('/learner/learning-path') || pathname.startsWith('/learner/courses') || pathname.startsWith('/learner/quiz')) {
        return SECTIONS_CONFIG.learning;
      }
      if (pathname.startsWith('/learner/materials/upload') || pathname.startsWith('/learner/materials')) {
        return SECTIONS_CONFIG['ai-tools'];
      }
      if (pathname.startsWith('/learner/competencies')) {
        return SECTIONS_CONFIG.competency;
      }
      if (pathname.startsWith('/learner/analytics')) {
  ...
  const handleBack = () => {
    ...
    if (pathname.startsWith('/learner/courses/') && pathname !== '/learner/courses') {
      navigate('/learner/courses');
      return;
    }
    // 2. If inside a quiz take or result page -> return to Assessments list
    if (pathname.startsWith('/learner/quiz/') && pathname !== '/learner/quiz') {
      ...
      <div className="max-w-[1500px] w-full mx-auto px-4 sm:px-8 h-full flex items-center justify-between relative">
              ...
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-lg sm:text-xl font-extrabold text-white tracking-tight">Samarth</span>
                  </div>
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <button
            ...
            title="Open Samarth AI Learning Advisor"
          >
